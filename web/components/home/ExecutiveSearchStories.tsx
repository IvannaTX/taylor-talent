            ...
            {stages.map((stage, index) => (
              <div key={stage.label}>
                ...
                <span className="mt-2 block h-1 overflow-hidden rounded-full bg-sunken">
                  <motion.span
                    className="block h-full origin-left rounded-full bg-accent-indigo"
                    initial={{ scaleX: 0 }}
                    whileInView={{ scaleX: parseInt(stage.width, 10) / 100 }}
                    viewport={{ once: true }}
          ...
          <div className="mt-7 space-y-2 border-t border-line pt-4">
            {["Final conversation", "Leadership references", "Offer alignment"].map((item, index) => (
              <div key={item} className="flex items-center justify-between rounded-lg border border-line bg-raised px-3 py-2.5 text-xs text-muted">
                <span>{item}</span>
                <span className={cn("font-mono text-[0.5rem] uppercase tracking-[0.1em]", index === 0 ? "text-ok" : "text-faint")}>{index === 0 ? "Active" : "Ready"}</span>
              </div>
            ))}
          </div>
        </div>
...
export function ExecutiveSearchStories() {
  ...
  const paused = manualPaused || hovering || !inView;
  ...
  React.useEffect(() => {
    ...
    const controls = animate(progress, 1, {
      duration: (1 - progress.get()) * (STORY_MS / 1000),
      ease: "linear",
      onComplete: () => setActive((current) => (current + 1) % searchStories.length),
    });
    return () => controls.stop();
  }, [active, paused, progress, reduce]);

  return (
    <Reveal dir="up" blur={false} className="mt-6 sm:mt-8">
      <section
        ref={sectionRef}
        aria-roledescription="carousel"
        aria-label="Why companies choose Taylor Talent"
        className="overflow-hidden rounded-[1.5rem] border border-line bg-surface shadow-card"
        ...
        <div className="grid min-h-[34rem] lg:grid-cols-[0.72fr_1.28fr]">
          <div className="flex min-h-[25rem] flex-col border-b border-line p-6 sm:p-8 lg:min-h-0 lg:border-b-0 lg:border-r lg:p-10">
            <AnimatePresence mode="wait" initial={false}>
              <motion.div key={story.id} initial={{ opacity: 0, y: 14, scale: 0.992 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -10, scale: 0.995 }} transition={{ duration: 0.6, ease: EASE }} className="flex flex-1 flex-col">
            ...
            <div className="mt-8 flex items-center gap-3" aria-label="Choose a story">
              <div className="flex items-center gap-2 rounded-pill border border-line bg-raised p-2">
                {searchStories.map((item, index) => {
                  ...
                  return (
                    <button key={item.id} type="button" aria-label={`Show ${item.title}`} aria-current={current ? "true" : undefined} onClick={() => setActive(index)} className={cn("relative h-2 overflow-hidden rounded-full transition-[width,background-color] duration-500 ease-apple", current ? "w-10 bg-ink/20" : "w-2 bg-faint/50 hover:bg-muted")}>
                      {current ? <motion.span className="absolute inset-0 block rounded-full bg-ink" style={{ scaleX: progress, originX: 0 }} /> : null}
                    </button>
                  );
                })}
              </div>
              <button type="button" onClick={() => setManualPaused((value) => !value)} aria-label={manualPaused ? "Play carousel" : "Pause carousel"} className="grid h-10 w-10 place-items-center rounded-full border border-line bg-raised text-muted transition-colors hover:text-ink">
          ...
          <AnimatePresence mode="wait" initial={false}>
            <motion.div key={story.id} initial={{ opacity: 0, x: 22, scale: 1.01 }} animate={{ opacity: 1, x: 0, scale: 1 }} exit={{ opacity: 0, x: -14, scale: 1.005 }} transition={{ duration: 0.68, ease: EASE }} className="min-h-[22rem]">
